...
month <- as.numeric(format(monthly_maxima$date, "%m"))
year  <- as.numeric(format(monthly_maxima$date, "%Y"))

# Composante de tendance (temps centré sur janvier 1996, exprimé en fraction de siècle)
time_trend <- (month + 12 * (year - 1996)) / (12 * 100)

# Composantes saisonnières
cosines <- cos(2 * pi * (month - 1) / 12)
sines   <- sin(2 * pi * (month - 1) / 12)

# Fit GEV avec non-stationnarité dans la location
fit_nonstat <- gev.fit(xdat = monthly_maxima$value,
                       ydat = matrix(c(time_trend, cosines, sines), ncol = 3),
                       mul = c(1:3))
